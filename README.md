...
SDD-TDD Paradigm（规格驱动 × 测试驱动开发范式）是一个分形结构的多阶段开发技能。它根据变更规模自适应选择开发路径，大改动走完整的 5 阶段流程（Explore → Proposal → Apply(TDD) → Review(多Agent交叉验证) → Archive），小改动走普通流程。通过决策前置、实现验证、多Agent审查的三重保障，确保大改动的质量和可追溯性。
...
发现：函数 handlePayment 未校验 amount 是否为负数
  ...
  └─ 仅被 1 Agent 标记
      ├─ 指派辩护 Agent 尝试反驳
      │   → 找到反驳证据：第 37 行有 if (amount 
<一句话描述要做什么>

## 影响范围
| 文件/模块 | 变更类型 | 说明 |
|---------|---------|------|
| src/auth/jwt.rs | 修改 | 需要扩展 token 生成逻辑 |
| src/api/login.rs | 新增 | 新的登录接口 |
...
## 未知项
- 旧版客户端的具体版本号需要和客户端团队确认
- bcrypt 的 cost factor 对性能的影响需要做 benchmark

## 关键发现
- 当前的 auth 模块没有 token 刷新机制，这是一个潜在的扩展点
- 数据库里已有 users 表，可以直接用
```
